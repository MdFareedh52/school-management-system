...
class Student{
	...
	public function getStudents(){
		$query = "SELECT * FROM tbl_student2";
	...
	public function insertStudent($name, $roll){
		...
		if (empty($name) || empty($roll)) {
			...
			$stu_query = "INSERT INTO tbl_student2(name, roll) VALUES('$name', '$roll')";
			$stu_insert = $this->db->insert($stu_query);

			$att_query = "INSERT INTO tbl_attendance2(roll) VALUES('$roll')";
	...
	public function insertAttendance($attend = array()){
		$query = "SELECT DISTINCT att_time FROM tbl_attendance2";
		...
		foreach ($attend as $atn_key => $atn_value) {
			if ($atn_value == "present") {
				$stu_query = "INSERT INTO tbl_attendance2(roll, attend, att_time) VALUES('$atn_key', 'present', now())";
				$data_insert = $this->db->insert($stu_query);
			} elseif ($atn_value == "absent") {
				$stu_query = "INSERT INTO tbl_attendance2(roll, attend, att_time) VALUES('$atn_key', 'absent', now())";
				$data_insert = $this->db->insert($stu_query);
			}
		}
	...
	public function getAllData($dt){
		...
		$query = "SELECT tbl_student2.name, tbl_attendance2.*
				FROM tbl_student2
				INNER JOIN tbl_attendance2
				ON tbl_student2.roll = tbl_attendance2.roll
				WHERE att_time = '$date'";
		$result = $this->db->select($query);
		return $result;
	}
